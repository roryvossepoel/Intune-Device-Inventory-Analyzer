...
export default function SmartTable<T>({rows,columns,rowKey,exportName,onRowClick,initialPageSize=50}:Props<T>){
  ...
  return <>
    {isDevices&&<div className="deviceFilterShell">
      <div className="deviceFilterTop"><div><strong>Explore devices</strong><span>{sorted.length.toLocaleString()} of {rows.length.toLocaleString()} devices{activeFilterCount?` · ${activeFilterCount} active filter${activeFilterCount===1?'':'s'}`:''}</span></div><div><button className={columnsOpen?'active':''} onClick={()=>setColumnsOpen(v=>!v)}>▦ Columns <b>{shownColumns.length}</b></button>{activeFilterCount>0&&<button className="clearExplorer" onClick={clearFilters}>Clear filters</button>}</div></div>
