...
const fmtPct=(value:number)=>{const rounded=Math.round(value*10)/10;return `${Number.isInteger(rounded)?rounded.toFixed(0):rounded.toFixed(1)}%`};
...
export function InventorySummary({devices}:{devices:Device[]}){
  ...
  if(topPlatform){const share=pct(topPlatform[1],total);introParts.push(`${platformName(topPlatform[0])} is the largest platform with ${fmt(topPlatform[1])} devices (${fmtPct(share)})`)}
  if(platformCounts.length>1){const rest=platformCounts.slice(1,4).map(([name,n])=>`${platformName(name)} ${fmtPct(pct(n,total))}`);if(rest.length)introParts.push(`followed by ${rest.join(', ')}`)}
  ...
  if(compliancePct>=90)healthParts.push(`Overall compliance is strong at ${fmtPct(compliancePct)}`);
  else if(compliancePct>=75)healthParts.push(`Overall compliance is ${fmtPct(compliancePct)}, with ${fmt(noncompliant)} noncompliant device${noncompliant===1?'':'s'} requiring attention`);
  else healthParts.push(`Compliance requires attention: ${fmtPct(compliancePct)} of devices are compliant and ${fmt(noncompliant)} are noncompliant`);
  if(stale30>0){const staleShare=pct(stale30,total);healthParts.push(`${fmt(stale30)} device${stale30===1?' has':'s have'} not checked in for more than 30 days${stale90?`, including ${fmt(stale90)} inactive for over 90 days`:''} (${fmtPct(staleShare)} of inventory)`)}
  if(knownEncryption){const encPct=pct(encrypted,knownEncryption);if(encPct>=98)healthParts.push(`reported encryption coverage is very strong at ${fmtPct(encPct)}`);else if(encPct>=90)healthParts.push(`reported encryption coverage is ${fmtPct(encPct)}`);else healthParts.push(`reported encryption coverage is ${fmtPct(encPct)}, which warrants review`)}
...
export default function EncryptionCard({devices,onEncrypted,onNotEncrypted,onUnknown}:{devices:Device[];onEncrypted?:()=>void;onNotEncrypted?:()=>void;onUnknown?:()=>void}){let encrypted=0,notEncrypted=0,unknown=0;for(const d of devices){const s=encryptionState(d);if(s===true)encrypted++;else if(s===false)notEncrypted++;else unknown++}const total=devices.length,p=total?encrypted/total*100:0;const iconTone=p>=98?'good':p>=90?'warn':'bad';const parts=[['Encrypted',encrypted,'good'],['Not encrypted',notEncrypted,'bad'],['Unknown',unknown,'neutral']] as const;let cursor=0;const colors={good:'#18a873',bad:'#e45d61',neutral:'#9aa9ba'};const stops=parts.filter(([,n])=>n>0).map(([,n,t])=>{const start=cursor;cursor+=total?n/total*100:0;return `${colors[t]} ${start}% ${cursor}%`});return <><InventorySummary devices={devices}/><article className="dashboardCard encryptionCard"><header className="dashboardCardHead securityCardHead"><span className={`securityCardIcon ${iconTone}`}><LockIcon/></span><div><h2>Encryption status</h2><p>Reported device encryption state</p></div></header><div className="encryptionBody"><div className="securityDonut" style={{background:`conic-gradient(${stops.join(',')||'#e7eef7 0 100%'})`}}><div><strong>{fmtPct(p)}</strong><span>encrypted</span></div></div><div className="securityLegend">{parts.filter(([,n])=>n>0).map(([label,n,t])=>{const action=label==='Encrypted'?onEncrypted:label==='Not encrypted'?onNotEncrypted:label==='Unknown'?onUnknown:undefined;return <button key={label} className={t} disabled={!action} onClick={action}><i/><span>{label}</span><strong>{fmt(n)}</strong><small>{fmtPct(total?n/total*100:0)}</small></button>})}</div></div></article></>}
