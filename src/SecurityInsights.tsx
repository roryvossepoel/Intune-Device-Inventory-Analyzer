...
function platformName(value:string){return value==='windows'?'Windows':value==='android'?'Android':value==='macos'?'macOS':value==='linux'?'Linux':value==='applemobile'?'Apple Mobile':'Unknown'}
...
function InventorySummary({devices}:{devices:Device[]}){
  ...
  return <article className="inventorySummaryCard">
    <header><span className="inventorySummaryIcon"><SummaryIcon/></span><div><h2>Inventory summary</h2><p>Automatically composed from the current dashboard scope.</p></div><span className="inventorySummaryBadge">Current view</span></header>
...
function SummaryIcon(){return <svg viewBox="0 0 24 24"><path d="M5 4h14v16H5z"/><path d="M8 8h8M8 12h8M8 16h5"/></svg>}
