...
function ensureOverlay(){
  ...
  backdrop.innerHTML=`<section class="importProgressPanel" role="dialog" aria-modal="true" aria-label="Processing inventory export">
    ...
    <div class="importProgressBody"><div class="importProgressCurrent"><div><strong></strong><span></span></div><b></b></div><div class="importProgressTrack" aria-hidden="true"><i></i></div><div class="importProgressSteps"></div></div>
...
function renderSteps(progress:ImportProgress,ready=false){
  ...
  holder.replaceChildren(...steps.map((step,index)=>{
    ...
    title.textContent=step.label;description.textContent=step.description;text.append(title,description);row.append(state,text);return row;
  }));
}
...
function render(progress:ImportProgress){
  ...
  if(progress.stage==='error'){
    panel.classList.add('importFailed');headTitle.textContent='Could not process this export';body.hidden=true;error.hidden=false;
    error.querySelector('p')!.textContent=progress.detail||'The export could not be read.';
    return;
  }

  panel.classList.remove('importFailed');headTitle.textContent='Analyzing your inventory';body.hidden=false;error.hidden=true;
  ...
  if(progress.stage==='build'){
    ...
    finishTimer=window.setTimeout(()=>showReady(progress),260);
  }
}
...
function showReady(progress:ImportProgress){
  ...
  headTitle.textContent='Inventory ready';currentLabel.textContent='Dashboard generated';currentDetail.textContent='Your inventory is ready to explore';currentPercent.textContent='100%';track.style.width='100%';renderSteps(progress,true);
  closeTimer=window.setTimeout(hideOverlay,420);
}
