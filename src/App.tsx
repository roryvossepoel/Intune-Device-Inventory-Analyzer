...
export default function App(){
  ...
  const scoped = useMemo(()=>base.filter(d=>{ if(!filter) return true; const value = filter.field==='compliance' ? d.compliance : filter.field==='osVersion' ? d.osVersion : filter.field==='manufacturer' ? d.manufacturer : filter.field==='model' ? d.model : (d.userUpn||d.userDisplayName); return key(value)===filter.value; }),[base,filter]);
...
function Overview({devices,allDevices,total,users,models,compliant,noncompliant,grace,stale,unknownModels,noUser,compliance,manufacturers,platforms,platform,onPlatform,drill}:{devices:Device[];allDevices:Device[];total:number;users:number;models:number;compliant:number;noncompliant:number;grace:number;stale:number;unknownModels:number;noUser:number;compliance:[string,number][];manufacturers:[string,number][];platforms:[string,number][];platform:string|null;onPlatform:(p:string|null)=>void;drill:(f:NonNullable<Filter>['field'],l:string,v:string)=>void}){
  ...
  const missingSerial=devices.filter(d=>!d.serialNumber||!d.serialNumber.trim()).length,missingModel=unknownModels,completeModel=total-missingModel,completeSerial=total-missingSerial,withUser=total-noUser;
  const qualityChecks=[{label:'Hardware model',complete:completeModel,total},{label:'Serial number',complete:completeSerial,total},{label:'Primary user',complete:withUser,total}];
