...
export default function App(){
  ...
  return <div className="app">
    ...
    {view==='faq'?<FaqPage/>:!data?<main className="landing landingPro">
      ...
      {view==='devices'&&<DataCard title="Device inventory" subtitle=""><SmartTable rows={deviceRows} columns={deviceColumns} rowKey={d=>d.id} exportName="intune-devices" onRowClick={setSelected} initialFilters={deviceInitialFilters} onClearFilters={()=>{setPlatform(null);setFilter(null)}} searchQuery={query} onClearSearch={()=>setQuery('')}/></DataCard>}
...
function Overview({devices,allDevices,total,users,models,compliant,noncompliant,grace,stale,compliance,platforms,platform,onPlatform,drill}:{devices:Device[];allDevices:Device[];total:number;users:number;models:number;compliant:number;noncompliant:number;grace:number;stale:number;compliance:[string,number][];platforms:[string,number][];platform:string|null;onPlatform:(p:string|null)=>void;drill:(f:NonNullable<Filter>['field'],l:string,v:string)=>void}){
  ...
  return <div className="inventoryDashboard">
    ...
    <DashboardSection icon="security" title="Health & attention" subtitle="Prioritized device risks, compliance, encryption and recent activity.">
    ...
    <DashboardSection icon="fleet" title="Fleet composition" subtitle="Device types, ownership, manufacturers, models and operating-system adoption.">
      <ExtendedInsights devices={devices} showPlatformDistribution={false}/>
    </DashboardSection>
...
function Search({value,setValue}:{value:string;setValue:(v:string)=>void}){return <div className="search"><span className="searchIcon" aria-hidden="true">⌕</span><input value={value} onChange={e=>setValue(e.target.value)} placeholder="Search…"/>{value&&<button type="button" className="searchClear" onClick={()=>setValue('')} aria-label="Clear search"><svg aria-hidden="true" viewBox="0 0 16 16"><path d="m4.5 4.5 7 7M11.5 4.5l-7 7"/></svg></button>}</div>}
