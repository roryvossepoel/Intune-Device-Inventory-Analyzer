...
const platformLabel:Record<string,string>={windows:'Windows',android:'Android',ios:'iOS',ipados:'iPadOS',iosipados:'iOS/iPadOS',macos:'macOS',linux:'Linux',unknown:'Unknown'};
...
export default function ExtendedInsights({devices}:{devices:Device[]}){
  ...
  return <section className="insightCategoryStack">
    <InsightCategory title="Hardware" subtitle="Form factor, standardization, manufacturers and model distribution.">
      <div className="extendedInsightGrid">
        ...
        <InsightCard icon="standardize" title="Hardware standardization" subtitle="How concentrated the fleet is around common models"><div className="metricTiles"><Metric label="Unique models" value={fmt(modelCounts.length)}/><Metric label="Top 10 coverage" value={pct(top10,total)}/><Metric label="One-off models" value={fmt(singletonModels)}/></div><div className="miniBars">{modelCounts.slice(0,5).map(([model,n])=><div key={model}><span className="truncate">{model}</span><strong>{fmt(n)}</strong><i><b style={{width:pct(n,modelCounts[0]?.[1]||1)}}/></i></div>)}</div></InsightCard>
